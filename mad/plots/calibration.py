...
def make_plots(save, bin_size, xaxis, dist):
    ...
    # df = df.sort_values(by=['ares',xaxis,  dist])
    df = df.sort_values(by=[dist, 'ares',  xaxis  ])

    if ('pdf' in dist) or ('logpdf' in dist) or ('oneClassSVM' in dist) :
        sign = -1.0
    ...
    if dist == 'cosine_simsiam':

        xmin, xmax, ymin, ymax = pl.axis()

        # if xmax > 0.0001:
        #     xmax = 0.0001
        # ax_err.set_xlim(xmin-0.00001, xmax + 0.00001)
