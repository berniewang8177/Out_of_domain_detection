...
from mad.ml import distances, distances_nn_cosine, distances_cosine_ctr
...
class dist_func_model_cosine_nn:
    ...
    def train(self, X, model, y=None):
        self.dist_func = lambda x: distances_nn_cosine.distance(X, x, model = model)
...
class builder:
    ...
    def nestedcv(
        ...
        if model_type in ensemble_methods:
            ...
            model.fit(repr_tr_cosine, label)

            dists_nn_cosine.train(X = repr_tr_cosine, model = model ) # 1 is in-domain, 0 for OOD
            # dist score is the probability to be OOD
            tr_distance_cosine = dists_nn_cosine.predict(repr_tr_cosine) # in domain training distances
            te_distance_cosine = dists_nn_cosine.predict(repr_te_cosine)
            # assert False f""
            for key in tr_distance_cosine:
                df_td[key] = tr_distance_cosine[key]
            for key in te_distance_cosine:
