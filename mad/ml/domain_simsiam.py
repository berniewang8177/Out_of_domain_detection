...
from mad.ml import distances, distances_simsiam
...
from mad.representation.get_repr_simsiam import train_representation_simsiam
import warnings
...
class dist_func_model_simsiam:

    def train(self, X, y=None):
        self.dist_func = lambda x: distances_simsiam.distance(X, x, y)
...
class builder:
    ...
    def nestedcv(
        ...
        if model_type in ensemble_methods:
            ...
            repr_model_simsiam = train_representation_simsiam(train_ctr, label, dataset_name, 'cosine')
            if dataset_name == 'fried':
                ...
                select_dim = 22
            repr_tr_simsiam = extract_embeddings(train_ctr[:,:select_dim],repr_model_simsiam ) # np.array type
            repr_te_simsiam = extract_embeddings(test_ctr[:,:select_dim],repr_model_simsiam )

            dists_ctr_simsiam = dist_func_model_simsiam()
            # train VS. train
            dists_ctr_simsiam.train(repr_tr_simsiam)
            tr_distance_simsiam = dists_ctr_simsiam.predict(repr_tr_simsiam)
            # train VS. test
            te_distance_simsiam = dists_ctr_simsiam.predict(repr_te_simsiam )
            for key in tr_distance_simsiam:
                df_td[key] = tr_distance_simsiam[key]
            for key in te_distance_simsiam:
                df_id[key] = te_distance_simsiam[key]
