...
from mad.ml import distances, distances_maha_ctr, distances_cosine_ctr, distances_att_ctr
...
class dist_func_model_att_ctr:

    def train(self, X0, X1, y=None):
        self.dist_func = lambda x0, x1: distances_att_ctr.distance(X0, x0, X1, x1, y)

    def predict(self, x0, x1):
        return self.dist_func(x0, x1)
...
class builder:
    ...
    def nestedcv(
        ...
        if model_type in ensemble_methods:
            ...
            repr_model_cosine = train_representation_cosine(train_ctr, label, dataset_name, 'cosine')
            repr_tr_cosine = extract_embeddings(train_ctr,repr_model_cosine) # np.array type
            repr_te_cosine = extract_embeddings(test_ctr,repr_model_cosine)
            # BECAREFUL df_id now is just a dictionary!
            repr_tr_id_cosine = repr_tr_cosine[id_mask]
            repr_tr_ood_cosine = repr_tr_cosine[ood_mask ]
            dists_ctr_cosine = dist_func_model_cosine_ctr()
            # dits1 = train_id VS. All train
            dists_ctr_cosine.train(repr_tr_id_cosine)
            tr_distance_cosine = dists_ctr_cosine.predict(repr_tr_cosine) # in domain training distances
            # dits2 = train_id VS. test
            te_distance_cosine = dists_ctr_cosine.predict(repr_te_cosine)

            # insert modified distances
            for key in tr_distance_cosine:
                df_td[key] = tr_distance_cosine[key]
            for key in te_distance_cosine:
                df_id[key] = te_distance_cosine[key]

            # train representation for mahalanobis
            repr_model_maha = train_representation_maha(train_ctr, label, dataset_name, 'mahalanobis')
            repr_tr_maha = extract_embeddings(train_ctr,repr_model_maha) # np.array type
            repr_te_maha = extract_embeddings(test_ctr,repr_model_maha)
            repr_tr_id_maha = repr_tr_maha[id_mask]
            repr_tr_ood_maha = repr_tr_maha[ood_mask ]
            dists_ctr_maha = dist_func_model_maha_ctr()
            # dits1 = train_id VS. All train
            dists_ctr_maha.train(repr_tr_id_maha)
            tr_distance_maha = dists_ctr_maha.predict(repr_tr_maha) # in domain training distances
            # dits2 = train_id VS. test
            te_distance_maha = dists_ctr_maha.predict(repr_te_maha)
            for key in tr_distance_maha:
                df_td[key] = tr_distance_maha[key]
            for key in te_distance_maha:
                df_id[key] = te_distance_maha[key]

            # use repr for maha and repr for cosine, together, form the attention metric
            dists_ctr_att = dist_func_model_att_ctr()
            dists_ctr_att.train(repr_tr_id_cosine, repr_tr_id_maha)
            tr_distance_att = dists_ctr_att.predict(repr_tr_cosine, repr_tr_maha)
            te_distance_att = dists_ctr_att.predict(repr_te_cosine, repr_te_maha)
            for key in tr_distance_att:
                df_td[key] = tr_distance_att[key]
            for key in te_distance_att:
                df_id[key] = te_distance_att[key]
