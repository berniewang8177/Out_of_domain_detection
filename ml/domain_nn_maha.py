...
from mad.ml import distances, distances_maha_ctr, distances_nn_maha
...
class dist_func_model_maha_nn:

    def train(self, X, model, y=None):
        self.dist_func = lambda x: distances_nn_maha.distance(X, x, model = model)
...
class builder:
    ...
    def nestedcv(
        ...
        if model_type in ensemble_methods:
            ...
            case_by_case_err = abs(stdcal_id_cv)
            ...
            dists_nn_maha = dist_func_model_maha_nn()
            ...
            model.fit(repr_tr_maha, label)

            dists_nn_maha.train(X = repr_tr_maha, model = model ) # 1 is in-domain, 0 for OOD
            # dist score is the probability to be OOD
            tr_distance_maha = dists_nn_maha.predict(repr_tr_maha) # in domain training distances
            te_distance_maha = dists_nn_maha.predict(repr_te_maha)
            # assert False f""
            for key in tr_distance_maha:
                df_td[key] = tr_distance_maha[key]
            for key in te_distance_maha:
